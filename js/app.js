...
const I18N = {
  en: {
    ...
    arunodayaTermDescription: "Pre-dawn period used for Ekadashi purity rules; it is calculated as one fifteenth of the previous night before sunrise.",
    paranaTerm: "Parana",
  ...
  ru: {
    ...
    arunodayaTermDescription: "Предрассветный период для правил чистоты Экадаши; считается как одна пятнадцатая предыдущей ночи до восхода.",
    paranaTerm: "Паран",
...
function renderTithiMuhurtaPanel(day, model) {
  ...
  return `
    <section class="tithi-muhurta-panel">
      ...
      ${renderAmritaMahendraSection(day)}
    </section>
  `;
}

function renderAmritaMahendraSection(day) {
  const data = day.jyotish?.amrita_mahendra;
  const source = AMRITA_MAHENDRA_SOURCE.i18n?.[currentLanguage] || AMRITA_MAHENDRA_SOURCE.i18n.en;
  if (!data) return renderYogaResearchNote(source);
  return `
    <div class="jyotish-section amrita-mahendra-section">
      <h4>${source.title}</h4>
      <div class="amrita-mahendra-grid">
        ${renderYogaWindowGroup(tr("amritaYogaTerm"), tr("yogaDaytime"), data.amritaDay, day.location.timezone)}
        ${renderYogaWindowGroup(tr("amritaYogaTerm"), tr("yogaNighttime"), data.amritaNight, day.location.timezone)}
        ${renderYogaWindowGroup(tr("mahendraYogaTerm"), tr("yogaDaytime"), data.mahendraDay, day.location.timezone)}
...
function renderYogaWindowGroup(title, period, ranges, timezone) {
  const values = ranges?.length ? ranges.map((range) => yogaRangeText(range, timezone)).join(", ") : tr("noYogaWindows");
  return `
    <div class="yoga-window-card">
...
function yogaRangeText(range, timezone) {
  return `${calendarTime(range.start, timezone)}-${calendarTime(range.end, timezone)}`;
}

function renderYogaResearchNote(source = AMRITA_MAHENDRA_SOURCE.i18n.en) {
  return `
    <aside class="jyotish-research-note">
      <strong>${source.title || tr("yogaResearchNoteTitle")}</strong>
...
function nominativeVaishnavaPersonLabel(value) {
  ...
  const exact = new Map([
    ["Абхирамы Тхакура", "Абхирам Тхакур"],
    ["Бхугарбхи Госвами", "Бхугарбха Госвами"],
    ["Вамшидаса Бабаджи", "Вамшидас Бабаджи"],
    ["Вамшиваданы Тхакура", "Вамшивадан Тхакур"],
    ["Васудевы Гхоша", "Васудев Гхош"],
    ["Вишванатхи Чакраварти Тхакура", "Вишванатх Чакраварти Тхакур"],
    ["Вишнуприи Деви", "Вишнуприя Деви"],
    ["Вриндавана Даса Тхакура", "Вриндаван Дас Тхакур"],
    ["Вриндавана даса Тхакура", "Вриндаван Дас Тхакур"],
    ["Гададхары Даса Госвами", "Гададхар Дас Госвами"],
    ["Гададхары Пандита", "Гададхар Пандит"],
    ["Гауры Кишоры Даса Бабаджи", "Гаура Кишор Дас Бабаджи"],
    ["Гауридаса Пандита", "Гауридас Пандит"],
    ["Гопалы Бхатты Госвами", "Гопал Бхатта Госвами"],
    ["Джаядевы Госвами", "Джаядев Госвами"],
    ["Дхананджая Пандита", "Дхананджай Пандит"],
    ["Калии Кришнадаса", "Калия Кришнадас"],
    ["Кашишвары Пандита", "Кашишвар Пандит"],
    ["Кришнадаса Кавираджа Госвами", "Кришнадас Кавирадж Госвами"],
    ["Локанатхи Госвами", "Локанатх Госвами"],
    ["Лочаны Даса Тхакура", "Лочан Дас Тхакур"],
    ["Мадхавендры Пури", "Мадхавендра Пури"],
    ["Мадхавы Ачарьи", "Мадхава Ачарья"],
    ["Мукунды Датты", "Мукунда Датта"],
    ["Мурари Гупты", "Мурари Гупта"],
    ["Нароттамы Даса Тхакура", "Нароттам Дас Тхакур"],
    ["Нимбарки Ачарьи", "Нимбарка Ачарья"],
    ["Парамешвары Даса Тхакура", "Парамешвар Дас Тхакур"],
    ["Пундарики Видьянидхи", "Пундарика Видьянидхи"],
    ["Пурушоттамы Даса Тхакура", "Пурушоттам Дас Тхакур"],
    ["Радха Кунды, Снана Дана", "Радха Кунда, Снана Дана"],
    ["Радхаштами - день явления Шримати Радхарани", "Шримати Радхарани"],
    ["Рагхунанданы Тхакура", "Рагхунандан Тхакур"],
    ["Рагхунатхи Бхатты Госвами", "Рагхунатх Бхатта Госвами"],
    ["Рагхунатхи Даса Госвами", "Рагхунатх Дас Госвами"],
    ["Рамананды Рая", "Рамананда Рай"],
    ["Рамануджи Ачарьи", "Рамануджа Ачарья"],
    ["Расикананды", "Расикананда"],
    ["Сварупы Дамодары Госвами", "Сваруп Дамодар Госвами"],
    ["Шанкарачарьи", "Шанкарачарья"],
    ["Шаранги Тхакура", "Шаранга Тхакур"],
    ["Шивананды Сены", "Шивананда Сен"],
    ["Шри Адвайты Ачарьи", "Шри Адвайта Ачарья"],
    ["Шри Ганги Маты Госвамини", "Шри Ганга Мата Госвамини"],
    ["Шривасы Пандита", "Шривас Пандит"],
    ["Шридхары Пандита", "Шридхар Пандит"],
    ["Шримати Гангаматы Госвамини", "Шримати Гангамата Госвамини"],
    ["Шримати Джахнавы Деви", "Шримати Джахнава Деви"],
    ["Шримати Ситы Деви", "Шримати Сита Деви"],
    ["Шрилы Баладева Видьябхушана Прабху", "Шрила Баладев Видьябхушан Прабху"],
    [
    ...
    [
      "Шрилы Бхакти Прапанны Тиртхи Госвами Махараджа",
      "Шрила Бхакти Прапанна Тиртха Госвами Махарадж"
    ],
    [
      "Шрилы Бхакти Сиддханты Сарасвати Госвами Прабхупада",
      "Шрила Бхакти Сиддханта Сарасвати Госвами Прабхупад"
    ],
    ["Шрилы Бхакти Винода Тхакура", "Шрила Бхакти Винод Тхакур"],
    ["Шрилы Бхактиведанты Свами Махараджа Прабхупада", "Шрила Бхактиведанта Свами Махарадж Прабхупад"],
    ["Шрилы Вакрешвара Пандита", "Шрила Вакрешвар Пандит"],
    ["Шрилы Вриндавана Даса Тхакура", "Шрила Вриндаван Дас Тхакур"],
    ["Шрилы Вьясадева", "Шрила Вьясадев"],
    ["Шрилы Гададхара Пандита", "Шрила Гададхар Пандит"],
    ["Шрилы Говинды Гхоша Тхакура", "Шрила Говинда Гхош Тхакур"],
    ["Шрилы Гопала Бхатты Госвами Прабху", "Шрила Гопал Бхатта Госвами Прабху"],
    ["Шрилы Джаганнатха Даса Бабаджи Махараджа", "Шрила Джаганнатх Дас Бабаджи Махарадж"],
    ["Шрилы Дживы Госвами Прабху", "Шрила Джива Госвами Прабху"],
    ["Шрилы Нарахари Саркара Тхакура", "Шрила Нарахари Саркар Тхакур"],
    ["Шрилы Нароттама Даса Тхакура", "Шрила Нароттам Дас Тхакур"],
    ["Шрилы Прабхупады", "Шрила Прабхупада"],
    [
      "Шрилы Рагхунатха Даса Госвами, Шрилы Рагхунатха Бхатты Госвами и Шрилы Кришнадаса Кавираджа Госвами",
      "Шрила Рагхунатх Дас Госвами, Шрила Рагхунатх Бхатта Госвами и Шрила Кришнадас Кавирадж Госвами"
    ],
    ["Шрилы Рамачандры Кавираджа", "Шрила Рамачандра Кавирадж"],
    ["Шрилы Рупы Госвами и Шрилы Гаури Даса Пандита Госвами", "Шрила Рупа Госвами и Шрила Гаури Дас Пандит Госвами"],
    ["Шрилы Санатана Госвами Прабху", "Шрила Санатан Госвами Прабху"],
    ["Шрилы Харидаса Тхакура", "Шрила Харидас Тхакур"],
    ["Шрилы Шриниваса Ачарьи", "Шрила Шринивас Ачарья"],
    ["Шрилы Гададхара Пандита и Шрилы Бхакти Винода Тхакура", "Шрила Гададхар Пандит и Шрила Бхакти Винод Тхакур"],
    ["Шьямананды Прабху", "Шьямананда Прабху"]
  ]);
